...
function Presenter(options) {
	...
	this.inputView
		...
		.on('blur', function() {
			var wasOpen = self.dropDownView.isVisible();
			setTimeout(function() { //wait for the click event to fire before hiding the list

				//if the user has indicated a selected suggestion and then clicked tab, then select it
				if (self.selectItemWhenTabPressed && wasOpen && self.tabPressed) {
					self.complete();
					self.emit('blur');
				...
				if (!(wasOpen && !self.dropDownView.isVisible())) {
					//emit the blur event if no option was selected - if an option was selected the control will be focused again
