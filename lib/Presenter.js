...
function Presenter(options) {
	...
	this.inputView.on('key', function (key) {
		console.log(self.listView.isVisible());
		if (self.listView.isVisible()) {
			...
			switch (key) {
				...
				case InputView.KEY_DOWN:
					console.log(self.listView.getSelectedIndex());
					if (self.listView.getSelectedIndex() === null) {
				...
				case InputView.KEY_DELETE:
				case InputView.KEY_BACKSPACE:
					self.autocomplete();
					break;
...
Presenter.prototype.autocomplete = function() {
	var self    = this;
	var query   = self.inputView.getQuery();
