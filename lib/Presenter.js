...
function Presenter(options) {
	 ...
	 */
	this.display = options.display || 'toString';

	/**
	 *
	 * @type {function(string):boolean}
	 */
	this.canSuggest = options.canSuggest;

	//set the minimum number of characters a user query must be before any suggestions are fetched
	if (typeof(options.canSuggest) === 'undefined') {
		this.canSuggest = function(query) {
			return query.length >= (options.minLength || 3);
		};
	};
...
Presenter.prototype.suggest = function() {
	...
	//ensure we can provide suggestions for the query
	if (!this.canSuggest(query)) {
		self.close();
		return self;
	}
