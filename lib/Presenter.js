...
function Presenter(options) {
	...
	this.inputView.on('key', function (key) {
		...
		if (key === InputView.KEY_DELETE || key === InputView.KEY_BACKSPACE) {
			self.suggest();
		} else if (self.dropDownView.isVisible()) {
			switch (key) {
	...
	this.inputView.el.parentNode.insertBefore(this.dropDownView.el, this.dropDownView.nextSibling);
}
...
Presenter.prototype.suggest = function() {
	...
	if (query === this.query) {
		self.dropDownView.setVisible(true);
		return self;
	}
